...
**目標**：針對自己企業、熟悉的品牌，或第 1 週選定的產業情境，規劃一套社群輿情監控與危機應對的初步流程。
...
本週不安排正式的「論文研討」場次，但可以用一篇跟本週主題直接相關的真實論文，練習精讀與簡易重現：

| 論文標題 | 期刊／年份 | 網址 |
|---|---|---|
| AI-Based Sentiment Analysis of E-Commerce Customer Feedback: A Bilingual Parallel Study on the Fast Food Industry in Turkish and English（本週精讀範例） | *Journal of Theoretical and Applied Electronic Commerce Research* (MDPI), 2025, 20(4), 294 | <https://www.mdpi.com/0718-1876/20/4/294> |

### 教師示範精讀：一個容易被忽略的陷阱——自動標註不等於真的準確

**① 研究問題與資料**：作者分析三個知名速食品牌（McDonald's、Burger King、KFC）在 X（Twitter）上的顧客留言，蒐集了 **14 萬 5 千多則英文推文與 1 萬 5 千多則土耳其文推文**，比較不同語言的顧客情緒表現差異。

**② 方法**：比較四種傳統機器學習模型（單純貝氏、SVM、羅吉斯迴歸、隨機森林）與一個 BERT 深度學習模型，BERT 表現最好（土耳其文 macro-F1 約 0.88），隨機森林則是傳統模型中表現最好的。

**③ 全篇最值得管理者警惕的發現**：由於人工逐則標註 14 萬筆推文不切實際，作者用「詞典式（lexicon-based）自動標註」的方式先產生訓練標籤，模型在這些自動標籤上驗證，準確率超過 95%；但當作者請人工重新檢查一小部分樣本、比對「自動標籤」跟「人工真正判斷」是否一致時，發現**英文只有 75%、土耳其文只有 49% 真正吻合**。也就是說，模型学得很好的，其實有很大一部分是在複製自動標註工具本身的偏誤與錯誤，而不是真正理解顧客情緒。

**④ 額外發現**：英文推文的情緒比較兩極化（明顯正面或負面），土耳其文推文則絕大多數呈現中性——這反映了不同語言╱文化在網路上表達情緒的方式不同，不能假設「這套情感分析模型在 A 語言／市場好用，換到 B 語言／市場也一樣準」。

**這篇論文對我們的啟示**：情感分析系統的「準確率 95%」這種數字，一定要問清楚「是跟什麼比較出來的準確率」——如果只是跟另一個自動化工具的標籤比較，這個數字可能大幅高估了模型的真實可靠度。**這正是各位在導讀任何一篇報告高準確率的論文時，都應該養成的懷疑習慣**：這個準確率的「地面真相（ground truth）」到底是怎麼來的？

### Vibe Coding 簡易重現：自動標籤 vs. 人工判斷，差距有多大？

**題目定義**：接續本週 Part 2 的餐廳評論情感分析模型，模擬論文發現的落差——比較「模型自己給自己打分」與「真正的人工標籤」之間可能存在的差距。

**給 AI 的提示詞：**
```
接續 Part 2 範例①訓練好的餐廳評論情感分析模型，請幫我做以下事情：
1. 從測試集中隨機抽出 20 筆評論
2. 印出這 20 筆評論的原文、模型預測的情緒標籤，以及資料集裡原本的
   真實標籤（Liked 欄位）
3. 計算這 20 筆裡，模型預測跟真實標籤一致的比例
4. 特別列出預測錯誤的評論，讓我可以逐一閱讀，判斷是模型判斷錯誤，
   還是這則評論本身的情緒其實很模糊、難以用單純的正負面二分法歸類
請給我可以在 Google Colab 直接執行的完整程式碼。
```

**延伸練習**：讀一讀被模型判斷錯誤的評論，看看是不是有「反諷」「先褒後貶」這類人類讀起來都容易誤判的複雜語句——這正是情感分析技術目前最大的限制之一。

**管理意涵**：這個練習讓各位親手體驗論文揭示的問題——即使模型在測試集上算出來的整體準確率很高，個別案例攤開來看，還是會有明顯誤判，而這些誤判經常發生在語意本來就比較細膩、模糊的句子上。企業導入情感分析系統做輿情監控時，不能只看整體準確率數字，還需要定期人工抽查，才能真正掌握系統的可靠度。
...
3. 把 Part 4 的人工核對範例也跑過一次，並找時間把速食品牌情感分析那篇論文完整讀一遍。
4. 鎖定 1–2 篇 Round 2 候選論文，準備下週的選題諮詢。可參考的搜尋方向：GenAI 對組織生產力的影響、LLM 於專業領域的決策支援、企業 RAG 知識庫構建、AI Agent 於供應鏈的應用——這幾個主題會分別對應第 11 至 14 週的內容。
